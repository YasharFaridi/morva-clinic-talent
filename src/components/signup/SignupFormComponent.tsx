...
export default function SignupPage() {
  ...
  const onSubmit = async (data: IUserFormData) => {
    ...
    try {
      const res = await signup(data as SignupRequest);
      ...
      if (err instanceof Error) serverMsg = err.message;
      else {
        const maybe = err as { response?: { data?: { message?: string } } };
        serverMsg = maybe?.response?.data?.message;
      }
      setErrorMessage(serverMsg || "خطا در ثبت نام");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center relative overflow-hidden bg-[url('/images/Form-bg.png')] bg-repeat bg-[length:256px_256px] bg-center">
